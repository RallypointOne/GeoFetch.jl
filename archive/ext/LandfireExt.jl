...
import GeoFetch: AbstractDataSource, DataAccessPlan, MetaData, RequestInfo,
...
```julia
using GeoFetch, Landfire, Rasters
...
function GeoFetch.fetch(plan::DataAccessPlan{LandfireSource})
    data = _dataset(plan)
    tif = Base.get(data)
    [tif]
end

function GeoFetch.load(plan::DataAccessPlan{LandfireSource})
    files = GeoFetch.fetch(plan)
    Raster(files[1])
end
