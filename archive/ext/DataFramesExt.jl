...
import GeoFetch: DataAccessPlan, load, fetch

using GeoFetch: OpenMeteoArchive, OpenMeteoForecast, NOAANCEI, NASAPower, TomorrowIO,
    VisualCrossing, USGSEarthquake, USGSWaterServices, OpenAQ, EPAAQS,
    OpenStreetMap

using GeoFetch: JSON3
...
function GeoFetch.load(plan::DataAccessPlan{<:Union{OpenMeteoArchive.Source, OpenMeteoForecast.Source}})
    files = fetch(plan)
...
function GeoFetch.load(plan::DataAccessPlan{NOAANCEI.Source})
    files = fetch(plan)
...
function GeoFetch.load(plan::DataAccessPlan{NASAPower.Source})
    files = fetch(plan)
    dfs = DataFrame[]
...
function GeoFetch.load(plan::DataAccessPlan{TomorrowIO.Source})
    files = fetch(plan)
    all_dfs = DataFrame[]
    for (i, file) in enumerate(files)
        json = JSON3.read(read(file, String))
...
function GeoFetch.load(plan::DataAccessPlan{VisualCrossing.Source})
    files = fetch(plan)
...
function GeoFetch.load(plan::DataAccessPlan{USGSEarthquake.Source})
    files = fetch(plan)
    all_dfs = DataFrame[]
    for file in files
        json = JSON3.read(read(file, String))
...
function GeoFetch.load(plan::DataAccessPlan{USGSWaterServices.Source})
    files = fetch(plan)
    all_rows = NamedTuple[]
    for file in files
        json = JSON3.read(read(file, String))
...
function GeoFetch.load(plan::DataAccessPlan{OpenAQ.Source})
    files = fetch(plan)
    all_rows = NamedTuple[]
    for file in files
        json = JSON3.read(read(file, String))
...
function GeoFetch.load(plan::DataAccessPlan{EPAAQS.Source})
    files = fetch(plan)
    all_dfs = DataFrame[]
    for file in files
        json = JSON3.read(read(file, String))
...
function GeoFetch.load(plan::DataAccessPlan{OpenStreetMap.Source})
    files = fetch(plan)
    all_rows = NamedTuple[]
